...
/**
 * SiteCrowd — the walking-crowd visual effect, built on the same Skiper39
 * CrowdCanvas primitive as before (components/ui/skiper39.tsx, untouched).
 * This previously shipped as WorkforceCrowd with a fabricated "1,200 people
 * / 42+ active sites" claim, which is why the whole section was removed
 * during the honesty pass — the animation itself was never the problem.
 * Re-added with copy that doesn't claim a number BuildMart doesn't have.
 */
export function SiteCrowd() {
  ...
  return (
    <section className="relative h-[720px] w-full overflow-hidden bg-navy-950 text-[#faf7f0]">
      ...
      <div
        ...
        style={{
          background: "radial-gradient(ellipse 900px 500px at 50% 0%, rgba(169,128,63,.16), transparent 60%)",
        }}
      />
      ...
      <div className="relative z-10 mx-auto grid max-w-5xl gap-4 px-6 pt-20 text-center sm:pt-24">
        ...
        <div className="mx-auto -mx-4 rounded-2xl bg-navy-950/55 px-4 py-3 backdrop-blur-[2px] sm:mx-auto sm:bg-transparent sm:px-0 sm:py-0 sm:backdrop-blur-none">
          ...
          <h2 className="mt-2 font-display text-3xl font-semibold tracking-tight text-[#faf7f0] sm:mt-0 sm:text-5xl">
            Every project runs on people, not just materials.
          </h2>
          <p className="mx-auto mt-2 max-w-2xl text-balance text-sm text-[#faf7f0]/65 sm:mt-0 sm:text-base">
            Engineers, site supervisors and skilled crews keep a build moving — BuildMart&apos;s job is making sure
            the materials they need show up on time, from a supplier that&apos;s actually nearby.
          </p>
        </div>
      </div>
      ...
      <div
        ...
        style={{ background: "linear-gradient(0deg, rgba(12,20,32,.9), transparent)" }}
      />
    </section>
  );
}

export default SiteCrowd;
